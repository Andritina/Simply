...
import { ChevronDown, User, Settings, HelpCircle, LogOut, Clock } from 'lucide-react';
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-72 bg-gradient-to-b from-slate-800 to-slate-900 rounded-xl shadow-2xl border border-slate-800/30 overflow-hidden z-10 animate-in slide-in-from-top-2 duration-200 ">
              ...
              {/* Menu items */}
              <div className="py-2">
                <button className="w-full px-5 py-2.5 hover:bg-slate-700/60 text-left flex items-center gap-3 group transition-all duration-200">
                  <div className="p-1.5 rounded-lg bg-slate-800 group-hover:bg-blue-500/20 transition-colors">
                    <User size={16} className="text-slate-400 group-hover:text-blue-400 transition-colors" />
                  </div>
                  <span className="text-sm text-slate-300 group-hover:text-slate-100 transition-colors">Edit profile</span>
                </button>
                ...
                <button className="w-full px-5 py-2.5 hover:bg-slate-700/60 text-left flex items-center gap-3 group transition-all duration-200">
                  ...
                  <span className="text-sm text-slate-300 group-hover:text-slate-100 transition-colors">Account settings</span>
                </button>

                <button className="w-full px-5 py-2.5 hover:bg-slate-700/60 text-left flex items-center gap-3 group transition-all duration-200">
                  <div className="p-1.5 rounded-lg bg-slate-800 group-hover:bg-green-500/20 transition-colors">
                    <HelpCircle size={16} className="text-slate-400 group-hover:text-green-400 transition-colors" />
                  </div>
                  <span className="text-sm text-slate-300 group-hover:text-slate-100 transition-colors">Support</span>
                </button>
              </div>

              {/* Sign out */}
              <div className="border-t border-slate-800/30 p-2">
                <button className="w-full px-5 py-2.5 hover:bg-red-500/10 text-left flex items-center gap-3 group transition-all duration-200 rounded-lg">
